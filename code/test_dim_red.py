...
model_list= [
  ('ShallowConvNet', ShallowConvNet(hidden_channels=60,linear_layer=4140),
                                                        '/WAVE/users/unix/smadsen/Desktop/bci_final/models/to_test/shallow/'),
  ('DeepConvNet',    DeepConvNet(linear_layer=1800),    '/WAVE/users/unix/smadsen/Desktop/bci_final/models/to_test/deep/'),
  ('EEGNet',         EEGNet(linear_layer=1125),         '/WAVE/users/unix/smadsen/Desktop/bci_final/models/to_test/eegnet/'),
  ('EEGInception',   EEGInception(input_channel=30),    '/WAVE/users/unix/smadsen/Desktop/bci_final/models/to_test/inception/')
]
...
for dataset_name, test_data_path in dataset_list:
  ...
  for model_name, model, weights_path in model_list:
    ...
    test_loader = DataLoader(test_dataset, batch_size=90, shuffle=True, pin_memory=True, num_workers=2)
