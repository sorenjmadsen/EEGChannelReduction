...
model_list= [
  ('ShallowConvNet', ShallowConvNet(input_channels=30, hidden_channels=60, linear_layer=4140), 
                                                      '/WAVE/users/unix/smadsen/Desktop/bci_final/models/to_test/shallow/'),
  ('DeepConvNet',    DeepConvNet(input_channels=30, linear_layer=1800), 
                                                      '/WAVE/users/unix/smadsen/Desktop/bci_final/models/to_test/deep/'),
  ('EEGNet',         EEGNet(input_channels=30, linear_layer=1125), 
                                                      '/WAVE/users/unix/smadsen/Desktop/bci_final/models/to_test/eegnet/'),
  ('EEGInception',   EEGInception(input_channel=30),  
...
for dataset_name, test_data_path in dataset_list:
  ...
  if len(test_data_files) == 0:
  	encode_trials_from_data(test_data_path +'test/', data_path, encoder)
  	test_data_files = glob.glob(data_path+ '/*.data')
